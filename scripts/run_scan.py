...
from redact import redact_secrets, REDACTED  # noqa: E402
...
def _ocr_thunk(
    ...
    Three modes:
      ...
      - ``scan`` + ``diff_files``: scan only the specified files (from
        --diff-only mode). Files are passed as comma-separated --path.
    ...
    def _run():
        ...
        if ocr_mode == "review":
            ...
            if diff_files:
                ...
                changed_rel = []
                for f in diff_files:
                    rel = os.path.relpath(f, target) if os.path.isabs(f) else f
                    if os.path.splitext(rel)[1] in OCR_EXTS:
                        changed_rel.append(rel)
            else:
                changed_rel = []

            if changed_rel:
                # Diff-only mode: scan only changed source files in batches of 10
                print(f"  OCR: scanning {len(changed_rel)} changed file(s) only")
                all_findings = []
                failed_files = set()
                BATCH_SIZE = 10
                for batch_start in range(0, len(changed_rel), BATCH_SIZE):
                    ...
                    if batch_start + BATCH_SIZE < len(changed_rel):
                ...
                # Full scan mode: batch by subdirectory (original behavior)
...
# Raw tool outputs written into the results dir that carry source-line or
# credential material verbatim:
#   - gitleaks.json: Secret / Match fields hold the credential itself.
#   - trufflehog.jsonl: Raw / RawV2 / Redacted fields hold the credential.
#   - bandit.json: results[].code holds the offending source lines.
#   - semgrep.sarif / semgrep-agent.sarif: results[].region.snippet.text
#     holds the offending source lines.
# The parsers in generate_report.py never copy these values into reports,
# but the raw files themselves sit on disk from the moment the scan writes
# them until someone cleans the results dir. The sweep below re-runs the
# shared redact.py rules over them IN PLACE once the pipeline finishes, so
# the on-disk raw outputs match SKILL.md's no-secret-on-disk guarantee.
_RAW_SECRET_OUTPUTS = (
    "gitleaks.json",
    "trufflehog.jsonl",
    "bandit.json",
...
# Fields whose entire value is credential material *by construction* (not
# just by pattern match). Regex rules can miss novel secret formats; schema
# knowledge cannot — these fields are replaced wholesale instead of being
# passed through the pattern layer.
_SECRET_BY_SCHEMA_FIELDS = frozenset(
    {"Secret", "Match", "Raw", "RawV2", "Redacted", "PrivateKey"}
)


def _redact_json_strings(obj, parent_key=None):
    """Recursively redact every string leaf of a parsed JSON structure.

    Known secret-schema fields (gitleaks ``Secret``/``Match``, trufflehog
    ``Raw``/``RawV2``/``Redacted``) are replaced wholesale with ``[REDACTED]``;
    every other string (bandit ``code``, SARIF ``snippet.text``, ...) goes
    through the shared redact_secrets rules, which preserve surrounding
    context for triage.
    """
    if isinstance(obj, dict):
        return {k: _redact_json_strings(v, parent_key=k) for k, v in obj.items()}
    if isinstance(obj, list):
        return [_redact_json_strings(v, parent_key) for v in obj]
    if isinstance(obj, str):
        if parent_key in _SECRET_BY_SCHEMA_FIELDS and obj.strip():
            return REDACTED
        return redact_secrets(obj)
    return obj


def _redact_raw_outputs(out_dir):
    """Redact secret-bearing raw tool outputs in the results dir, in place.

    Best-effort per file: a file that cannot be parsed or rewritten is
    reported on stderr and skipped so one malformed output can't block the
    sweep of the others. Files whose content is already clean are left
    untouched (re-running over a redacted results dir is a no-op).

    Returns the list of files that were actually rewritten.
    """
    rewritten = []
    for name in _RAW_SECRET_OUTPUTS:
        ...
        try:
            ...
            if name.endswith(".jsonl"):
                ...
                for line in content.splitlines():
                    ...
                    if stripped.startswith("{"):
                        try:
                            line = json.dumps(
                                _redact_json_strings(json.loads(stripped)),
                                ensure_ascii=False,
                            )
                        except json.JSONDecodeError:
                            # Progress/preamble noise: still scrub the text.
                ...
                safe_content = (
                    json.dumps(_redact_json_strings(data), indent=2, ensure_ascii=False)
                    + "\n"
                )
            if safe_content == content:
                continue
            # Atomic write-back: write a sibling temp file then rename over
            # the original, so a crash mid-write can't leave a torn file.
            fd, tmp_path = tempfile.mkstemp(
                dir=out_dir, prefix=".redact-", suffix=".tmp"
            )
            try:
            ...
            print(f"warning: could not redact raw output {name}: {e}", file=sys.stderr)
    return rewritten
...
def main():
    ...
    # bandit/SARIF code snippets). The report parsers never copy those values,
    # but the raw files sit on disk — scrub them in place before handing the
    # results dir over to generate_report.py or anyone else.
    redacted_files = _redact_raw_outputs(out_dir)
    if redacted_files:
        print(f"Secret-on-disk sweep: redacted {', '.join(redacted_files)}")
