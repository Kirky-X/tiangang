...
class TestToolCountDeclaration(unittest.TestCase):
    """P2-8: skill.json / README.md / README_EN.md 应声明扫描器集合。

    工具格局（absorbed strix SCA/密钥通道后）：
    - 10 种语言专属扫描器（核心集合：Bandit/FindSecBugs/Gosec/…）
    - 全生态 SCA + 密钥扫描通道（trivy/gitleaks/trufflehog）
    """
    ...
    def test_actual_parser_count_matches(self):
        """PARSERS 应包含所有声明的扫描器（10 个核心语言专属 + SCA/密钥通道）。"""
        import generate_report
        keys = set(generate_report.PARSERS.keys())
        # 10 个核心语言专属（文档声明的集合）
        expected_lang_specific = {
            "bandit.json", "gosec.sarif", "flawfinder.sarif", "brakeman.sarif",
            "psalm.sarif", "findsecbugs.sarif", "cppcheck.xml", "cargo-audit.json",
            "security-code-scan.json", "miri.log",
        }
        # 通用 SCA + 密钥通道（absorbed from strix）
        expected_sca_secret = {
            "trivy.json", "trivy-version.json", "gitleaks.json", "trufflehog.jsonl",
        }
        for k in expected_lang_specific | expected_sca_secret:
            self.assertIn(k, keys, f"PARSERS 应注册 {k}")
        # 语言专属解析器至少 10 个（允许 javascript/retire 等扩展）
        universal = set(k for k in keys if k.startswith("semgrep")) | expected_sca_secret
        lang_specific = [k for k in generate_report.PARSERS if k not in universal]
        self.assertGreaterEqual(
            len(lang_specific), 10,
            f"至少 10 个语言专属解析器，实际 {len(lang_specific)}: {lang_specific}",
        )
